...
class InteractiveTemplateMatcher:
    ...
    def draw_results(self):
        """Tegner vores model's predictions ovre i original-billedet inklusiv dem vi godkender / "Gemmer"."""
    ...
    def _print_terminal_summary(self):
        """Udskriver log i terminalen over fundne kroner pr. matrix position."""
        if self.board_hsv is None:
            return

        h_b, w_b = self.board_hsv.shape[:2]
        th_g = h_b // 5
        tw_g = w_b // 5

        alle_kroner = {(r, c): 0 for r in range(5) for c in range(5)}
        for x, y, w, h, *_ in self.found_boxes:
            cx = x + w // 2
            cy = y + h // 2
            col_idx = min(cx // tw_g, 4)
            row_idx = min(cy // th_g, 4)
            alle_kroner[(row_idx, col_idx)] += 1

        rigtige_kroner = {pos: antal for pos, antal in alle_kroner.items() if antal > 0}
        board_nr = self.current_board_number()

        print(f"\nBoard {board_nr}  –  Total fundne kroner: {len(self.found_boxes)}")
        print("  Alle tiles (5×5 matrix):")
        for r in range(5):
            print("   ", [{(r, c): alle_kroner[(r, c)]} for c in range(5)])
        print(f"  Kun kroner  (rad,kol)→antal : {rigtige_kroner}")
    ...
    def save_crown(self, x: int, y: int):
        """Gemmer en matchet krone, hvis brugeren klikker på den."""
        for bx, by, bw, bh, _ in self.found_boxes:
            if bx <= x <= bx + bw and by <= y <= by + bh:
                box_key = (bx, by, bw, bh)
                if box_key in self.saved_boxes:
                    print("Allerede gemt – spring over")
                    break

                crop = self.board_orig[by : by + bh, bx : bx + bw]
                board_nr = self.current_board_number()
                n = len(list(self.kroner_dir.glob("*.jpg"))) + 1
                sti = self.kroner_dir / f"krone_board{board_nr}_{n:03d}.jpg"

                cv2.imwrite(str(sti), crop)
                self.saved_boxes.add(box_key)

                print(f"Krone gemt: {sti.name}")
                self.draw_results()
                break
    ...
    def _mouse_result_callback(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            self.save_crown(x, y)
    ...
    def run(self):
        ...
        cv2.setMouseCallback(self.window_result, self._mouse_result_callback)
